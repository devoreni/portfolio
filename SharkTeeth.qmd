...
- Real-World Performance
    - The model must outperform an inexperienced shark tooth hunter in the field.
- Security
...
**Model 2** was trained on a significantly expanded dataset. It had strong metrics and could locate shark teeth in photos where the tooth was obvious, but it still failed in real-world testing. Neither the testing, nor training images were representative of how a user would actually take a photo in the field.
...
This iterative process highlighted a fundamental lesson in applied machine learning: a model that performs well on a held-out test set is not guaranteed to perform well in deployment if the test set does not faithfully represent the deployment environment.
...
AWS Rekognition Custom Labels charges per inference hour — the model must be running to accept requests, and a running model accrues cost regardless of whether it is actively processing images. As the scope of real-world testing expanded, projected monthly costs grew to a point where further development could not be justified without a more deliberate cost management strategy. This was the primary reason the project was paused at its current state.

The total cost for this project was $1.28.
