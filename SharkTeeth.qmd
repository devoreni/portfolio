...
Flask is a lightweight Python web framework. The application factory pattern was used as it is the recommended approach for Flask applications that need to be portable across environments. Flask's extension ecosystem (`flask-login`, `flask-sqlalchemy`, `flask-wtf`) handles authentication, database access, and form validation respectively.
...
Domain Randomization is a machine learning technique in which parameters like lighting, texture, or physics are deliberately varied to help a model generalize. It could be used on training images to expand the dataset and improve accuracy. A collection of shark teeth images could be layered on top of images from the internet, then modified (stretched, rotated, grayscaled, contrast boost, etc.) to produce vast amounts of synthetic data.
...
The current model was trained exclusively on teeth and surroundings from a single location. It may not transfer well to sites with differently colored or textured sand, different rock compositions, or teeth from different species. Expanding the dataset to cover multiple locations would make the tool useful to shark tooth hunters broadly, rather than being tailored to one specific site.
